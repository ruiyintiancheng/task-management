<template>
  <div>
    <el-input v-model="paramData.value">
  ...
  methods: {
    setCron() {
      ...
      if (this.$refs.cron.activeName === 'cron') {
        this.paramData.value = this.$refs.cron.cronExpress
      } else {
        this.paramData.value = this.$refs.cron.simpExpress
      }
    ...
    openDialog() {
      ...
      this.$nextTick(function() {
        if (this.type === '1') {
          ...
          this.$refs.cron.setCornVal(this.paramData.value)
        } else if (this.type === '2') {
          this.$refs.cron.activeName = 'simp'
          // this.$refs.cron.setSimpVal('0/0')
          this.$refs.cron.setSimpVal(this.paramData.value)
        }
      })
    },
    changeSimp() {
      const result = this.paramData.value.split('/')

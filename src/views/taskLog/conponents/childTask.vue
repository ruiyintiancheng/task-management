        ...
        <el-table-column slot="optionColumn"
          ...
          <template slot-scope="scope">
            <el-button type="primary"
                       plain
                       size="mini"
                       v-if="scope.row.calStatus === '0'"
                       @click="setSuccess(scope.row)">设置成完成</el-button>
            <el-button type="primary"
                       plain
                       size="mini"
...
export default {
  ...
  methods: {
    ...
    setSuccess(row) {
      baseRequest('/basic/taskNodeSub/update', { 'subTaskId': row.subTaskId, 'calStatus': '5' }).then(response => {
        this.$message({
          type: 'success',
          message: '操作成功!'
        })
  ...
  computed: {
    getDefaultHeight() { // 获取表格高度
      return this.$store.state.app.containHeight * 0.8 - 330
    },
    getLogHeight() {
      return this.$store.state.app.containHeight * 0.8 - 100
    }
  },
  watch: {
  }
